...
def dark_fig(*args, **kwargs):
    ...
    try:
        ...
        logging.error(f"[error] Failed to create figure: {e}")
        return None
...
def dark_ax(ax):
    ...
    try:
        ...
        logging.error(f"[error] Failed to style axes: {e}")
    try:
        for spine in ax.spines.values():
            spine.set_edgecolor(GRID)
        ax.grid(color=GRID, linestyle="--", linewidth=0.5, alpha=0.6)
    except Exception as e:
        logging.error(f"[error] Failed to style grid: {e}")
    return ax
...
def load_result(symbol: str) -> dict | None:
    try:
        ...
        if not path.exists():
            logging.warning(f"[warn] No forecast file for {symbol}")
            return None
        with open(path) as f:
            return json.load(f)
    except Exception as e:
        logging.error(f"[error] Failed to load result for {symbol}: {e}")
        return None
...
def load_all_results() -> list[dict]:
    try:
        ...
        logging.error(f"[error] Failed to load all results: {e}")
        return []
...
def get_sector_map(con: duckdb.DuckDBPyConnection) -> dict:
    try:
        ...
        logging.error(f"[error] Failed to get sector map: {e}")
        return {}
...
def get_price_history(con: duckdb.DuckDBPyConnection,
    ...
    """).df()
        ...
        logging.error(f"[error] Failed to get price history for {symbol}: {e}")
        return pd.DataFrame()
...
def plot_ticker(symbol: str,
    ...
    try:
        ...
        if hist.empty:
            logging.warning(f"[warn] No price history for {symbol}")
            return None
    except Exception as e:
        logging.error(f"[error] Failed to get price history for {symbol}: {e}")
        return None
    ...
    try:
        ...
        if not forecast:
            logging.warning(f"[warn] No forecast data for {symbol}")
            return None
    except Exception as e:
        logging.error(f"[error] Failed to get forecast data for {symbol}: {e}")
        return None
    ...
    try:
        ...
        logging.error(f"[error] Failed to create figure for {symbol}: {e}")
        return None
    ...
    try:
        ...
        logging.error(f"[error] Failed to create axes for {symbol}: {e}")
        return None
    ...
    try:
        ...
        logging.error(f"[error] Failed to plot Bollinger bands for {symbol}: {e}")
        return None
    ...
    try:
        ...
        logging.error(f"[error] Failed to plot SMAs for {symbol}: {e}")
        return None
    ...
    try:
        ...
        logging.error(f"[error] Failed to plot price for {symbol}: {e}")
        return None
    ...
    try:
        ...
        logging.error(f"[error] Failed to plot vertical separator for {symbol}: {e}")
        return None
    ...
    try:
        ...
        logging.error(f"[error] Failed to plot GP forecast cone for {symbol}: {e}")
        return None
    ...
    try:
        ...
        logging.error(f"[error] Failed to plot GP mean for {symbol}: {e}")
        return None
    ...
    try:
        ...
        logging.error(f"[error] Failed to plot signal for {symbol}: {e}")
        return None
    ...
    try:
        ...
        logging.error(f"[error] Failed to set labels for {symbol}: {e}")
        return None
    try:
        plt.setp(ax_price.get_xticklabels(), visible=False)
    except Exception as e:
        logging.error(f"[error] Failed to set xticklabels for {symbol}: {e}")
        return None
    ...
    try:
        ...
        logging.error(f"[error] Failed to plot RSI for {symbol}: {e}")
        return None
    ...
    try:
        ...
        logging.error(f"[error] Failed to plot GP std for {symbol}: {e}")
        return None
    ...
    try:
        ...
        logging.error(f"[error] Failed to add uncertainty annotation for {symbol}: {e}")
        return None
    ...
    try:
        ...
        logging.error(f"[error] Failed to plot volume for {symbol}: {e}")
        return None
...
def plot_calibration(all_results: list[dict],
    ...
    try:
        ...
        logging.error(f"[error] Failed to plot calibration: {e}")
        return None
...
def plot_rmse_improvement(all_results: list[dict],
    ...
    try:
        ...
        logging.error(f"[error] Failed to plot RMSE improvement: {e}")
        return None
    ...
    if save:
        ...
        try:
            ...
            logging.error(f"[error] Failed to save RMSE improvement plot: {e}")
...
def plot_uncertainty_landscape(all_results: list[dict],
    ...
    if not rows:
        logging.warning("[warn] No uncertainty landscape data available")
        return None
...
def plot_aggregate(all_results: list[dict],
    ...
    logging.info("[plots] RMSE improvement by sector...")
    plot_rmse_improvement(all_results, sector_map)

    logging.info("[plots] Signal distribution...")
    plot_signal_distribution(all_results, sector_map)

    logging.info("[plots] Uncertainty landscape...")
...
def plot_selected_tickers(symbols: list[str],
    ...
    for symbol in symbols:
        result = load_result(symbol)
        if result:
            logging.info(f"[plots] Fan chart: {symbol}...")
...
def full_run():
    ...
    if all_results:
        ...
        logging.warning("[warn] No forecast results found in model_outputs/. "
